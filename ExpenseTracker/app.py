...
def dashboard():
    ...
    # Retrieve month from query parameters or session fallback
    ...
    if month is not None:
        ...
        month = session.get("selected_month", "").strip()
        if not month:
            month = now.strftime("%Y-%m")
    ...
    # Keep session value in sync with the validated month
    session["selected_month"] = month
...
def reports():
    ...
    if month_param is not None:
        ...
        # Check if year and month are individually in query parameters
        year_arg = request.args.get("year")
        month_arg = request.args.get("month")
        if year_arg and month_arg:
            try:
                month_param = f"{int(year_arg):04d}-{int(month_arg):02d}"
                session["selected_month"] = month_param
            except ValueError:
                month_param = session.get("selected_month", "").strip()
        else:
            month_param = session.get("selected_month", "").strip()
    ...
    # Keep session value in sync with the validated month
    session["selected_month"] = f"{year}-{month:02d}"
...
def budget():
    ...
    # Retrieve month from query parameters, POST data, or session fallback
    month = request.args.get("month")
    if month is None and request.method == "POST":
    ...
    try:
        ...
        if y > now.year or (y == now.year and m > now.month):
            ...
            session["selected_month"] = now.strftime("%Y-%m")
